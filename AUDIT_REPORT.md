...
### ⚠️ Real incident (2026-07-17): autopilot placed unauthenticated, oversized buy orders that were never closed

The repo owner reported that their live Bitget account showed a series of buy
orders with no corresponding sell-back, requiring manual intervention, and
that the system never actually enforced the take-profit/stop-loss levels it
displayed. Root-caused to three compounding bugs, all now fixed:

1. **[FIXED — CRITICAL]** `runAutopilotExecution()` in `src/utils/agent.ts` called `executeApprovedTrade()` **directly**, completely bypassing `AsiwajuAgentShield.processSecureTrade()` — meaning the $10 sizing cap, symbol whitelist, prompt-safety check, cooldown, and re-entrancy/replay guards **never ran for autopilot trades**. The only gate was a hardcoded `const confidenceScore = 9;` compared against `MIN_CONFIDENCE_REQUIRED = 8` — a constant that was never actually computed from anything, so it always passed.
   - Fix: `runAutopilotExecution()` now builds a `TradeRequest` and routes it through `AsiwajuAgentShield.processSecureTrade()`, identically to the manual-approval flow. The fake confidence check was removed.
2. **[FIXED — CRITICAL]** In `scanMarketOpportunity()`, buy-side proposals were sized with a hardcoded `proposal.quantity = "5.0000"` — i.e. **5 whole units** of BTC/ETH/SOL, not $5 worth. At the time, that's roughly $335,000 of BTC, $18,700 of ETH, or $860 of SOL per attempted buy — combined with bug #1 bypassing the $10 sizing cap entirely, autopilot could and did place wildly oversized market buy orders. The sell-side branch already correctly computed `5 / priceNum` (a ~$5 notional position); the buy-side branch never matched it.
   - Fix: both sides now use the same `5 / priceNum` sizing logic.
3. **[FIXED — FUNCTIONAL GAP]** There was, and had never been, any code anywhere that placed a stop-loss/take-profit order or monitored price to automatically close a position. `stopLoss`/`takeProfit` were purely cosmetic numbers shown in the UI/logs — nothing ever compared them to the live price or acted on them. This is why nothing sold back automatically.
   - Fix: added `src/utils/positions.ts` (in-memory open-position registry + a pure, thoroughly unit-tested `evaluateExitTrigger()` decision function) and `runPositionMonitorOnce()` in `src/utils/agent.ts`, wired into a new 60-second background loop in `src/index.ts`. Every successful **buy** executed through `AsiwajuAgentShield.processSecureTrade()` is now automatically registered for monitoring; when the live price crosses its stop-loss or take-profit, the monitor places a closing trade through the same Shield pipeline (with a new `isPositionExit` flag — see below) and stops tracking it.
   - **Known limitation, stated plainly**: this position registry is in-memory only and resets if the process restarts. It is a real improvement over "nothing at all," but it is not a substitute for exchange-native conditional (OCO/trigger) orders, which would survive a restart. It also has not been exercised against the real Bitget API from this development environment (no network access to `api.bitget.com` here) — the decision logic (`evaluateExitTrigger`) is unit tested exhaustively, but the live order-placement path should be verified manually with a small position before being trusted unattended.
4. **[FIXED — SAFETY]** Given the above, `runAutopilotExecution()` is now **disabled by default**. It requires an explicit `AUTOPILOT_ENABLED=true` environment variable to run at all, enforced at the single shared entry point used by the REST endpoint, the background scanner loop, and the Telegram/Discord bot commands, so no caller can bypass it. The new position-monitor loop (point 3) runs independently of this switch, since it only ever closes existing positions — it never opens new ones.
5. **[FIXED — separately, uncovered while writing tests for the above]** `scanMarketOpportunity()` also had a dead, redundant check — `if (!process.env.QWEN_API_KEY) throw ...` — placed immediately before its actual AI call. `callUnifiedAI()` (called two lines later) already implements proper Qwen→MuleRun failover with its own clear combined error. This dead check meant every scan (manual and autopilot) threw and refused to run whenever `QWEN_API_KEY` specifically was missing or exhausted, **even when `MULERUN_API_KEY` was correctly configured and would have worked fine** — a likely contributor to "doesn't work with actual analysis" reports. Removed, matching the equivalent fix already applied to `PromptFilter.ts`.

Also extended `RiskGuardrail.ts` with an `isPositionExit` option: closing an existing position is now exempt from the $10 sizing cap (a winning position can grow past $10 in value and must still be fully closeable) and from the 30-second cooldown (an unrelated trade elsewhere must never delay closing a position that's hit its stop) — while the symbol whitelist still applies. Blocking a legitimate exit would itself be a risk-management failure.

Verified: `tsc --noEmit` and `next build` both clean; 27/27 tests passing (10 pre-existing + 3 autopilot-Shield-routing regression tests in `agent.test.ts` + 9 position-registry tests + 5 new `isPositionExit` guardrail tests in `RiskGuardrail.test.ts`); confirmed live via a real server boot that the `/api/autopilot` endpoint now returns `AUTOPILOT_DISABLED` by default, and that the new 60-second position-monitor loop runs without error.
