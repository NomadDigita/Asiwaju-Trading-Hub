...
import { trackOpenPosition } from '../utils/positions';
...
export class AsiwajuAgentShield {
   ...
   * @param isPositionExit Set when this trade CLOSES an existing tracked position
   *   (a stop-loss or take-profit exit) rather than opening a new discretionary
   *   one. Exempts the trade from the $10 sizing cap and the cooldown — see
   *   GuardrailOptions in RiskGuardrail.ts for the full rationale. Defaults to
  ...
  public static async processSecureTrade(
    ...
    try {
      ...
      const guardrailReport = evaluateRiskGuardrails(tradeRequest, { isPositionExit });
      ...
      if (status === 'SUCCESS') {
        ...
        // Register this position for automatic stop-loss/take-profit
        // monitoring — see src/utils/positions.ts for the full rationale.
        // Only 'buy' entries are tracked: Bitget Spot has no shorting, so a
        // 'sell' here is always closing/reducing an existing holding (either
        // a manual sell or a position-exit routed back through this same
        // method with isPositionExit=true), not opening something new to
        // monitor.
        if (tradeRequest.side === 'buy' && !isPositionExit) {
          trackOpenPosition({
            symbol: tradeRequest.symbol,
            side: 'buy',
            quantity: tradeRequest.quantity,
            entryPrice: tradeRequest.price,
            stopLoss: parseFloat(proposal.stopLoss),
            takeProfit: parseFloat(proposal.takeProfit),
          });
          logs.push(`${getTimestamp()} 📌 AAS SHIELD: Position registered for automatic SL/TP monitoring (SL ${proposal.stopLoss}, TP ${proposal.takeProfit}).`);
        }
